...
if args.test_speed:
	...
	if args.parse_train_and_dev:
		...
		for index, subcorpus in enumerate(corpus.dev_list):
			...
			loader=ColumnDataLoader(list(subcorpus),eval_mini_batch_size,use_bert=student.use_bert, model = student, sort_data = not args.keep_order)
		...
		if args.recur_parse:
			...
			for file_dir in file_dirs:
				...
				loader=ColumnDataLoader(list(corpus.train),eval_mini_batch_size,use_bert=student.use_bert, model = student, sort_data = not args.keep_order)
				loader.assign_tags(student.tag_type,student.tag_dictionary)
		...
		loader=ColumnDataLoader(list(corpus.test),eval_mini_batch_size,use_bert=student.use_bert, model = student, sort_data = not args.keep_order)
		loader.assign_tags(student.tag_type,student.tag_dictionary)
